    ...
    <div>
        <div class="mb-5 text-center">
            ...
            <input
                ...
                class="hidden"
                type="file"
                @change="handleImageSelect('inputAvatar')"
            />
        </div>
            ...
            <div class="flex gap-4 justify-between items-center">
                <div>
                    ...
                    <input
                        ...
                        class="hidden"
                        type="file"
                        @change="handleImageSelect('inputNidFront')"
                    />
                </div>
                <div>
                    <img :src="nidFrontPlaceholder" class="rounded-md  w-100" />
                </div>
            </div>
        </div>

        <div class="mb-5">
            <label class="font-bold mb-1 text-gray-700 block">NID Image - Back Side</label>
            <div class="flex gap-4 justify-between items-center">
                <div>
                    <label
                        for="inputNidBack"
                    ...
                    <input
                        ...
                        class="hidden"
                        type="file"
                        @change="handleImageSelect('inputNidBack')"
                    />
                </div>
                <div>
                    <img :src="nidBackPlaceholder" class="rounded-md  w-100" />
                </div>
            </div>
        </div>
    </div>
...
<script>
import nidFrontPlaceholderImage from "../../images/nid_front_placeholder.png";
import nidBackPlaceholderImage from "../../images/nid_back_placeholder.png";
export default {
    name: "StepThree",
    props: ["createUserData"],
    data() {
        return {
            nidFrontPlaceholder: nidFrontPlaceholderImage,
            nidBackPlaceholder: nidBackPlaceholderImage,
            avatarPlaceholder:
    ...
    methods: {
        handleImageSelect(ref) {
            let file = document.getElementById(ref).files[0];

            if (ref === "inputAvatar") {
                this.createUserData.images.avatar = file;
            } else if (ref === "inputNidFront") {
                this.createUserData.images.nidFront = file;
            } else if (ref === "inputNidBack") {
                this.createUserData.images.nidBack = file;
            }

            var reader = new FileReader();
            reader.onload = e => {
                if (ref === "inputAvatar") {
                    this.avatarPlaceholder = e.target.result;
                } else if (ref === "inputNidFront") {
                    this.nidFrontPlaceholder = e.target.result;
                } else if (ref === "inputNidBack") {
                    this.nidBackPlaceholder = e.target.result;
                }
            };
